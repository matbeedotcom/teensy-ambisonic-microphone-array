/* Teensy 4.1 Master - Local I2S + Remote TDM to 12-Channel USB
 *
 * This sketch receives:
 * - 4 local I2S microphone channels (SAI1 RX)
 * - 8 remote TDM channels from Teensy B slave (SAI1 TX data input)
 * - Routes all 12 channels to USB output
 *
 ...
 * - Pin 8: SAI1_RXD0 - TDM data from slave (8 channels in slots 0-7)
 *
 ...
 */
 ...
 AudioOutputUSB       usbOut;         // 8-channel USB output to PC (max supported)
 ...
 // Connect remote TDM inputs to USB channels 4-11
 ...
 AudioConnection patchCord9(tdmIn, 4, usbOut, 8);       // Remote Ch4 (880 Hz)
 AudioConnection patchCord10(tdmIn, 5, usbOut, 9);      // Remote Ch5 (1047 Hz)
 AudioConnection patchCord11(tdmIn, 6, usbOut, 10);     // Remote Ch6 (1319 Hz)
 AudioConnection patchCord12(tdmIn, 7, usbOut, 11);     // Remote Ch7 (1568 Hz)

 // Monitor audio levels - local channels
 AudioAnalyzePeak     peakLocal[4];
 AudioConnection patchCord13(i2sQuadIn, 0, peakLocal[0], 0);
 AudioConnection patchCord14(i2sQuadIn, 1, peakLocal[1], 0);
 AudioConnection patchCord15(i2sQuadIn, 2, peakLocal[2], 0);
 AudioConnection patchCord16(i2sQuadIn, 3, peakLocal[3], 0);

 // Monitor audio levels - remote TDM channels
 AudioAnalyzePeak     peakRemote[8];
 AudioConnection patchCord17(tdmIn, 0, peakRemote[0], 0);
 AudioConnection patchCord18(tdmIn, 1, peakRemote[1], 0);
 AudioConnection patchCord19(tdmIn, 2, peakRemote[2], 0);
 AudioConnection patchCord20(tdmIn, 3, peakRemote[3], 0);
 AudioConnection patchCord21(tdmIn, 4, peakRemote[4], 0);
 AudioConnection patchCord22(tdmIn, 5, peakRemote[5], 0);
 AudioConnection patchCord23(tdmIn, 6, peakRemote[6], 0);
 AudioConnection patchCord24(tdmIn, 7, peakRemote[7], 0);
 ...
 void setup() {
   ...
   Serial.println("=============================================");
   Serial.println("Teensy 4.1 Master - 12-Channel USB Audio");
   Serial.println("=============================================");
   Serial.println();
   ...
   Serial.println();
   Serial.println("TDM from Teensy B slave (8 channels):");
   ...
   Serial.println("  Ch 4-11: Remote TDM from slave");
   Serial.println("    Ch 4:  440 Hz (A4)");
   Serial.println("    Ch 5:  523 Hz (C5)");
   Serial.println("    Ch 6:  659 Hz (E5)");
   Serial.println("    Ch 7:  784 Hz (G5)");
   Serial.println("    Ch 8:  880 Hz (A5)");
   Serial.println("    Ch 9:  1047 Hz (C6)");
   Serial.println("    Ch 10: 1319 Hz (E6)");
   Serial.println("    Ch 11: 1568 Hz (G6)");
   Serial.println();
   Serial.println("USB Audio: Should appear as 'Teensy Audio 12CH'");
   Serial.println("Starting 12-channel audio streaming...");
   Serial.println();
 }
     ...
     if (hasLocalData || hasRemoteData) {
       ...
       if (hasRemoteData) {
         ...
         for (int i = 0; i < 8; i++) {
           ...
           Serial.print("Ch");
           Serial.print(i + 4);  // USB channels 4-11
           Serial.print(":");
           printBarCompact(level);
           Serial.print(" ");
         }
